...
5. If the task touches Assistant/Comms, owner communication, Discord, cross-channel continuity, owner escalation/approval through communication channels, or the local Assistant interface, read `docs/superpowers/specs/2026-08-27-assistant-comms-architecture.md` and `docs/superpowers/plans/2026-08-27-assistant-comms.md`. Do not read these files for unrelated repository work merely because they exist.
...
- The current sequence is Task 7 Project Intelligence -> Task 8 Skills + MCP -> Task 9 Research Intelligence -> Task 10 Memory -> `V31M4-ASSISTANT-001` -> Task 11 Quality Floor -> Task 12 Eval Lab/sandbox bakeoff -> Task 13 Self-improvement. Assistant/Comms is a required current-system insertion and does not renumber the autonomy tasks.
...
- If live implementation evidence makes a v2/amendment/Assistant plan interface invalid, stop and record the exact conflict rather than silently redesigning the architecture or creating a shadow path.
...
## Assistant/Comms current-system rules

- `V31M4-ASSISTANT-001` is a required current-build program after Task 10 Memory and before Task 11 Quality Floor. It is not deferred to final productization.
- Assistant/Comms is an outer interface/application subsystem, not a department and not another agent framework.
- Discord is the approved initial free-first remote channel adapter, not a permanent dependency or authority; revalidate its live capabilities/policies when implementation begins.
- Assistant Runtime depends on a provider-neutral channel boundary; channel adapters cannot access runtime persistence directly or bypass Manager -> Executor -> Auditor, policy, approval, evidence, sandbox, or effect reconciliation.
- Conversation/channel history is provenance only, never authoritative task/memory/approval state.
- Answer / Task / Responsibility is the approved interaction model. First represent ongoing Responsibility through existing Mission + Scheduler + Task semantics; do not add a new authoritative Responsibility aggregate without proof it is needed.
- Outbound communication is a governed/reconciled external effect; crash/restart may not cause blind resend or duplicate material notification/approval prompts.
- Owner responses received through desktop/Discord/voice must bind to the exact current pending decision and be revalidated through existing approval/policy/current-state authority before a protected effect.
- Issue #10 still governs final one-click desktop packaging/startup, but the Assistant runtime/interface must already exist before that packaging step.
- Runtime API `1.0.0` remains unchanged unless a separately negotiated additive version is approved through `docs/contract-versioning.md`.
