...
4. If the change touches autonomous task state, Task DAGs, model/tool execution, sandboxing, project intelligence, context assembly, skills, memory, quality-floor execution, independent verification, MCP/A2A interoperability, or self-improvement, read `docs/superpowers/specs/2026-08-25-autonomy-quality-floor-architecture.md` and implement only the currently selected phase and its hard gate.
...
- Implement the autonomy phases in the dependency order defined by the spec. Do not begin self-improvement or later-phase optimization before earlier hard gates are satisfied.
