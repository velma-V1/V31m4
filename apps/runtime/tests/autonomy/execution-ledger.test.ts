...
/**
 * The canonical policy engine the semantic boundary now consults for itself. Task 1 final removed
 * the caller-asserted `policyDecision`, so authorization is only as strong as this engine's real
 * answer. It grants without an expiry, which is exactly the pre-integration behaviour: an
 * unexpiring grant makes `assertPolicyGrantCurrent` a no-op.
 */
const semanticPolicy: PolicyEnginePort = {
  async evaluate() {
    return Object.freeze({
      decision: "allow" as const,
      policyId: "policy:semantic-ledger-test",
...
describe("ambiguous effects block retry", () => {
  it("refuses the same intent after an indeterminate outcome", async () => {
    ...
    await expect(
      reconciler.runGovernedEffect(
        { taskId, sandbox, plan: await inspectPlan(), probe: appliedProbe },
        context,
      ),
  ...
  it("gives the same intent the same fingerprint across separate capabilities", async () => {
    expect(reconciler.intentFingerprintFor(await inspectPlan())).toBe(
      reconciler.intentFingerprintFor(await inspectPlan()),
    );
  });
});
...
describe("reconciling an attempt without re-dispatching", () => {
  async function attemptOnce(probe: EffectPostStateProbe): Promise<{
    readonly attemptEntryId: string;
    readonly plan: Awaited<ReturnType<typeof inspectPlan>>;
  }> {
    const plan = await inspectPlan();
...
describe("scoped identity must agree", () => {
  ...
  it("refuses a request whose taskId disagrees with the plan and sandbox", async () => {
    await expect(
      reconciler.runGovernedEffect(
        { taskId: otherTask, sandbox, plan: await inspectPlan(), probe: appliedProbe },
        context,
      ),
...
describe("decisions read the whole ledger", () => {
  ...
  it("still blocks on an unresolved attempt recorded past the first page", async () => {
    ...
    await recordAttempt("ledger:blocking", reconciler.intentFingerprintFor(await inspectPlan()));
  ...
  it("still blocks on a confirmed attempt recorded past the first page", async () => {
    await fillHistory(LONG_HISTORY);
    await recordAttempt("ledger:confirmed", reconciler.intentFingerprintFor(await inspectPlan()));
    await appendExecutionLedgerEntry(
      { unitOfWork: database.unitOfWork, ledger },
      ExecutionLedgerEntry.create({
...
describe("authoritative ledger state requires the canonical Task 1 issuer", () => {
   ...
   */
  async function clonedPlan(): Promise<Awaited<ReturnType<typeof inspectPlan>>> {
    return { ...(await inspectPlan()) } as Awaited<ReturnType<typeof inspectPlan>>;
  }
   ...
   */
  async function prototypeForgedPlan(): Promise<Awaited<ReturnType<typeof inspectPlan>>> {
    return Object.create(await inspectPlan()) as Awaited<ReturnType<typeof inspectPlan>>;
  }
  ...
  for (const [label, mint] of [
    ...
    it(`refuses ${label} on runGovernedEffect before any ledger write or dispatch`, async () => {
      await expect(
        reconciler.runGovernedEffect(
          { taskId, sandbox, plan: await mint(), probe: countingProbe(appliedProbe) },
          context,
        ),
      ).rejects.toMatchObject({ code: "PERMISSION_DENIED" });
  ...
  it("still runs the whole normal lifecycle for the canonical issuer", async () => {
    const outcome = await reconciler.runGovernedEffect(
      { taskId, sandbox, plan: await inspectPlan(), probe: countingProbe(appliedProbe) },
      context,
    );
    expect(outcome.outcomeKind).toBe("effect_confirmation");
