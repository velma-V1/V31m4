...
import { candidate, verification } from "../services/fixtures.js";
import { Harness, T0, context } from "./fixtures.js";
...
describe("champion, delivery, training, and promotion use cases", () => {
  it("stores a verified champion and complete delivery receipt", async () => {
    ...
    const contender = candidate("candidate-1");
    const result = verification("candidate-1", "passed");
    const decision = await selectChampionUseCase(
      { unitOfWork: harness.unitOfWork, candidates: harness.candidateRepository },
      {
        ...
        decidedAt: T0,
        assessments: [{ candidate: contender, verification: result, issues: [], correctness: 1, coverage: 1, security: 1, performance: 0.8, complexity: 0.2, evidenceStrength: 1 }],
      },
      context,
    );
    expect(decision.value.decision).toBe("champion");
    const receipt = await deliverResult(
      { unitOfWork: harness.unitOfWork, candidates: harness.candidateRepository, clock: harness.clock },
      { receiptId: "receipt-1", decision: decision.value, deliveredArtifactIds: [ArtifactId.parse("artifact-output-1")], requirementsCovered: 1, requirementsTotal: 1, mandatoryChecksPassed: 1, mandatoryChecksTotal: 1, unresolvedRiskIds: [], evidenceIds: [...decision.value.evidenceIds] },
      context,
    );
  ...
  it("keeps learning quarantined until verification, then promotes it with evidence-backed capability history", async () => {
    ...
    await harness.unitOfWork.execute(context, (transaction) => harness.trainingStore.save(verified, WriteConditions.matchRevision(packet.revision), context, transaction));
    const profile = CapabilityProfile.create({ capabilityId: "capability-1", displayName: "Capability One", domain: "software", initialScore: { capabilityId: "capability-1", score: 0.4, sampleSize: 4, difficultyRange: [0.2, 0.7], evidenceIds: ["evidence-old-1"], measuredAt: T0 } });
    harness.capabilities.set("capability-1", { value: profile, revision: "1" });
    const promotion = await promoteCapability(
      { unitOfWork: harness.unitOfWork, capabilities: harness.capabilityRepository, training: harness.trainingStore, clock: harness.clock },
      {
        ...
        observations: [
          { evidenceId: EvidenceId.parse("evidence-observation-1"), outcome: 1, difficulty: 0.8, measuredAt: T0, source: "production", leakageChecked: true },
          { evidenceId: EvidenceId.parse("evidence-observation-2"), outcome: 0.8, difficulty: 0.7, measuredAt: T0, source: "production", leakageChecked: true },
        ],
        minimumSampleSize: 2,
        practiceWeight: 0.25,
      },
