...
export function createRepositoryPorts(state: RepositoryState) {
  ...
  const approvalStore: ApprovalStorePort = {
    ...
    consume: async (id, condition) => {
      ...
      const stored = versioned(
        Object.freeze({ ...current.value, status: "consumed" as const }),
  ...
  const missionRepository: MissionRepositoryPort = {
    ...
    listByProject: async (projectId) => ({
      items: [...state.missions.values()].filter((item) => item.value.projectId === projectId),
    }),
  ...
  const jobRepository: JobRepositoryPort = {
    ...
    getLatestVerifiedCheckpoint: async (jobId) =>
      [...state.checkpoints.values()]
        .reverse()
        .find((item) => item.value.jobId === jobId && item.value.verified) ?? null,
  };
  ...
  const evidenceRepository: EvidenceRepositoryPort = {
    ...
    list: async (query) => ({
      items: [...state.evidenceRecords.values()].filter(
        (item) => query.statuses === undefined || query.statuses.includes(item.value.status),
      ),
    }),
    append: async (record) => {
  ...
  const candidateRepository: CandidateRepositoryPort = {
    ...
    listCandidates: async (missionId) => ({
      items: [...state.candidates.values()].filter((item) => item.value.missionId === missionId),
    }),
    ...
    listIssues: async (candidateId) => ({
      items: [...state.issues.values()].filter((item) => item.value.candidateId === candidateId),
    }),
  ...
  const trainingStore: TrainingStorePort = {
    ...
    list: async (status) => ({
      items: [...state.packets.values()].filter(
        (item) => status === undefined || item.value.status === status,
      ),
    }),
  ...
  const practiceRepository: PracticeRepositoryPort = {
    ...
    list: async (status) => ({
      items: [...state.practices.values()].filter(
        (item) => status === undefined || item.value.status === status,
      ),
    }),
