...
export function assertCondition<Value>(current: Versioned<Value> | undefined, condition: WriteCondition): void {
  if (condition.kind === "must_not_exist" && current !== undefined) throw new Error("record already exists");
  if (condition.kind === "match_revision" && current?.revision !== condition.revision) throw new Error("revision conflict");
}
...
export class TestUnitOfWork implements UnitOfWorkPort {
  ...
  async execute<Result>(operation: OperationContext, work: (transaction: UnitOfWorkTransaction) => Promise<Result>): Promise<Result> {
    ...
    const transaction: UnitOfWorkTransaction = {
      ...
      afterRollback(action) { afterRollback.push(action); },
    };
    try {
...
export class TestClock {
  ...
  now(): string { return this.values[Math.min(this.index++, this.values.length - 1)] ?? T2; }
  monotonicMilliseconds(): number { return this.index * 1_000; }
  async sleep(): Promise<void> {}
}

export function createEvidence(id: string, subjectId: string, subjectType = "acceptance_criterion"): EvidenceRecordType {
...
export function createVerification(plan: VerificationPlan, candidateId: string, evidenceId = `evidence-${candidateId}`): VerificationResultType {
