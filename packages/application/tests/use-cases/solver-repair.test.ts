...
function configuration() {
  return {
    ...
    strategy: "failure_first" as const,
    contextArtifactIds: [ArtifactId.parse("context-1")],
    toolIds: [ToolId.parse("tool-1")],
...
describe("solver, verification, issue, and repair use cases", () => {
  it("stores immutable candidates and independent verification evidence", async () => {
    ...
    if (candidate === undefined) throw new Error("Expected a solver candidate.");
    const outcomes = await verifyCandidates(
      {
        unitOfWork: harness.unitOfWork,
        evidence: harness.evidenceRepository,
        verifier: harness.verifier,
      },
      [{ plan: plan("plan-1", "candidate-1"), candidate }],
      context,
    );
  ...
  it("records an issue and persists a verified targeted repair", async () => {
    ...
    if (source === undefined) throw new Error("Expected a solver candidate.");
    await recordIssues(
      { unitOfWork: harness.unitOfWork, candidates: harness.candidateRepository },
      CandidateId.parse("candidate-1"),
      [
        {
          id: "issue-1",
          title: "Defect",
          exactDeficiency: "Output fails a required case.",
          severity: "high",
          evidenceIds: ["evidence-issue-1"],
          expectedConsequence: "Incorrect output.",
          proposedCorrection: "Repair exact case.",
    ...
    const outcome = await repairCandidate(
      ...
      },
      {
        projectId: ProjectId.parse("project-1"),
        jobId: JobId.parse("job-1"),
        issueId: IssueId.parse("issue-1"),
        sourceCandidate: source,
        repairedCandidateId: "candidate-2",
        repairId: "repair-1",
        invocationId: "invoke-repair",
        promptArtifactId: ArtifactId.parse("prompt-repair"),
        configuration: configuration(),
        focusedPlan: plan("focused", "candidate-2"),
        regressionPlan: plan("regression", "candidate-2"),
        createdAt: T0,
        resourceBudget: budget(),
      },
      context,
