import { Checkpoint, Job, type ArtifactId, type ContentHash, type EvidenceId, type JobId } from "@v31m4/domain";
...
export async function checkpointJob(
  ...
  await dependencies.unitOfWork.execute(context, async (transaction) => {
    const current = requireValue(await dependencies.jobs.getById(command.jobId, context, transaction), "Job does not exist.");
    const transitioning = Job.beginCheckpoint(current.value, { eventId: command.beginEventId, occurredAt: dependencies.clock.now(), stage: command.stage });
    await dependencies.jobs.save(transitioning.job, WriteConditions.matchRevision(current.revision), context, transaction);
  ...
  try {
    ...
    await dependencies.unitOfWork.execute(context, async (transaction) => {
      const current = requireValue(await dependencies.jobs.getById(command.jobId, context, transaction), "Checkpointing job disappeared.");
      const failed = Job.fail(current.value, { eventId: command.failureEventId, occurredAt: dependencies.clock.now(), failureReason: error instanceof Error ? error.message : "Kernel checkpoint failed." });
      await dependencies.jobs.save(failed.job, WriteConditions.matchRevision(current.revision), context, transaction);
  ...
  return dependencies.unitOfWork.execute(context, async (transaction) => {
    const current = requireValue(await dependencies.jobs.getById(command.jobId, context, transaction), "Checkpointing job disappeared.");
    const recorded = Job.recordCheckpoint(current.value, { eventId: command.recordedEventId, occurredAt: dependencies.clock.now(), checkpointId, stage: command.stage });
    const stored = await dependencies.jobs.appendCheckpoint(checkpoint, context, transaction);
    await dependencies.jobs.save(recorded.job, WriteConditions.matchRevision(current.revision), context, transaction);
