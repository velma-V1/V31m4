import { Job, type Job as JobType, type MissionId, type ProjectId, type ResourceBudget } from "@v31m4/domain";
...
export async function startJob(
  ...
  await dependencies.unitOfWork.execute(context, async (transaction) => {
    const stored = await dependencies.jobs.save(queued.job, WriteConditions.mustNotExist(), context, transaction);
  ...
  try {
    ...
    await dependencies.unitOfWork.execute(context, async (transaction) => {
      const current = requireValue(await dependencies.jobs.getById(queued.job.id, context, transaction), "Queued job disappeared after kernel failure.");
    ...
    throw new ApplicationError("DEPENDENCY_FAILURE", "Production kernel could not start the job.", { cause: error });
  }

  return dependencies.unitOfWork.execute(context, async (transaction) => {
    const current = requireValue(await dependencies.jobs.getById(queued.job.id, context, transaction), "Queued job disappeared after kernel acceptance.");
    const running = Job.start(current.value, {
      eventId: command.startedEventId,
      occurredAt: dependencies.clock.now(),
      stage: "running",
    });
    const stored = await dependencies.jobs.save(running.job, WriteConditions.matchRevision(current.revision), context, transaction);
