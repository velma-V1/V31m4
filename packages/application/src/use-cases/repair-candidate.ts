import {
  ...
  type ProjectId,
  RepairRecord,
  type RepairRecord as RepairRecordType,
  type ResourceBudget,
  SolverCandidate,
  ...
  try {
    ...
    if (invocation.finishReason !== "completed" || invocation.outputArtifactIds.length === 0) {
      throw new ApplicationError(
        "DEPENDENCY_FAILURE",
        "Repair invocation did not produce a candidate.",
      );
    }
    const candidate = SolverCandidate.createReconstructed(
      {
        ...
        configuration: {
          ...
          ...(command.configuration.temperature === undefined
            ? {}
            : { temperature: command.configuration.temperature }),
    ...
    assertApplication(
      command.focusedPlan.candidateId === candidate.id &&
        command.regressionPlan.candidateId === candidate.id,
      "INTEGRITY_FAILURE",
      "Repair verification plans must target the repaired candidate.",
    );
    const focused = await dependencies.verifier.execute(command.focusedPlan, candidate, context);
    const regression = await dependencies.verifier.execute(
      command.regressionPlan,
      candidate,
      context,
    );
    ...
    await dependencies.unitOfWork.execute(context, async (transaction) => {
      const storedIssue = requireValue(
        await dependencies.candidates.getIssue(command.issueId, context, transaction),
        "Repair issue does not exist.",
      );
      await dependencies.candidates.appendCandidate(candidate, context, transaction);
      for (const record of [...focused.evidence, ...regression.evidence])
      ...
      if (passed) {
        const accepted =
          storedIssue.value.status === "open"
            ? IssueRecord.accept(storedIssue.value)
            : storedIssue.value;
        const repairedIssue = IssueRecord.markRepaired(accepted);
        await dependencies.candidates.saveIssue(
          repairedIssue,
          WriteConditions.matchRevision(storedIssue.revision),
          context,
          transaction,
        );
      }
