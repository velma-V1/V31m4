import { SolverCandidate, type ArtifactId, type JobId, type MissionId, type ProjectId, type ResourceBudget, type SolverCandidate as SolverCandidateType, type SolverConfiguration } from "@v31m4/domain";
...
export async function runSolverForge(
  ...
  assertApplication(command.configurations.length > 0, "INVALID_APPLICATION_INPUT", "Solver forge requires at least one configuration.");
  assertApplication(command.candidateIds.length === command.configurations.length && command.invocationIds.length === command.configurations.length, "INVALID_APPLICATION_INPUT", "Every solver configuration requires a candidate and invocation ID.");
  assertApplication(new Set(command.candidateIds).size === command.candidateIds.length, "INVALID_APPLICATION_INPUT", "Candidate IDs must be unique.");
  ...
  for (let index = 0; index < command.configurations.length; index += 1) {
    ...
    assertApplication(configuration !== undefined && candidateId !== undefined && invocationId !== undefined, "INTEGRITY_FAILURE", "Solver forge input arrays became misaligned.");
    ...
    try {
      ...
      if (invocation.finishReason !== "completed" || invocation.outputArtifactIds.length === 0) {
        throw new ApplicationError("DEPENDENCY_FAILURE", "Solver invocation did not produce a complete candidate.", { details: { invocationId, finishReason: invocation.finishReason } });
      }
