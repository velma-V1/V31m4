    ...
    const evidence = await collectPortPages((cursor) =>
      dependencies.evidence.list(
        cursor === undefined
          ...
          : { limit: 10_000, cursor, statuses: ["passed"] },
        context,
        transaction,
      ),
    );
    const capabilityScores = Object.fromEntries(
      profiles.map((item) => [item.value.capabilityId, item.value.current.score]),
    );
    const evidenceByRule = Object.fromEntries(
      rules.map((rule) => [
        rule.id,
        evidence
          .map((item) => item.value)
          .filter(
            (record) =>
              ...
              new Set<string>(rule.requiredCapabilityIds).has(record.subjectId),
          )
          .map((record) => ({
