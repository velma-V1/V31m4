import type { EvidenceRecord, SolverCandidate, VerificationPlan, VerificationResult } from "@v31m4/domain";
...
export async function verifyCandidates(
  ...
  assertApplication(requests.length > 0, "INVALID_APPLICATION_INPUT", "At least one candidate verification is required.");
  const outcomes: CandidateVerificationOutcome[] = [];
  for (const request of requests) {
    throwIfOperationCancelled(context);
    assertApplication(request.plan.candidateId === request.candidate.id, "INTEGRITY_FAILURE", "Verification plan belongs to a different candidate.");
    const execution = await dependencies.verifier.execute(request.plan, request.candidate, context);
    assertApplication(execution.candidateId === request.candidate.id && execution.result.candidateId === request.candidate.id, "INTEGRITY_FAILURE", "Verifier returned a result for a different candidate.");
    ...
    outcomes.push(Object.freeze({ candidate: request.candidate, result: execution.result, evidence: Object.freeze([...execution.evidence]) }));
  }
