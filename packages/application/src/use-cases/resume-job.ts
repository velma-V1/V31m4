import { Job, type CheckpointId, type Job as JobType, type JobId } from "@v31m4/domain";
...
export async function resumeJob(
  ...
  const prepared = await dependencies.unitOfWork.execute(context, async (transaction) => {
    ...
    const checkpoint = command.checkpointId === undefined
      ? requireValue(await dependencies.jobs.getLatestVerifiedCheckpoint(command.jobId, context, transaction), "No verified checkpoint is available.")
      : requireValue(await dependencies.jobs.getCheckpoint(command.checkpointId, context, transaction), "Checkpoint does not exist.");
    ...
    const stored = await dependencies.jobs.save(resumed.job, WriteConditions.matchRevision(current.revision), context, transaction);
  ...
  try {
    ...
    await dependencies.unitOfWork.execute(context, async (transaction) => {
      const current = requireValue(await dependencies.jobs.getById(command.jobId, context, transaction), "Resumed job disappeared after kernel failure.");
    ...
    throw new ApplicationError("DEPENDENCY_FAILURE", "Production kernel could not resume the job.", { cause: error });
  }
}
