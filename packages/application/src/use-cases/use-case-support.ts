...
export async function authorizeAction(
  ...
  if (approvalId === undefined) {
    throw new ApplicationError("APPROVAL_REQUIRED", "This operation requires an explicit approval.", {
  ...
  if (stored === null || stored.value.status !== "granted" || Date.parse(stored.value.expiresAt) <= Date.parse(dependencies.clock.now())) {
    throw new ApplicationError("APPROVAL_REQUIRED", "The supplied approval is missing, expired, or not granted.", {
      details: { approvalId },
    });
  }
  const scopes = new Set(stored.value.requiredScopes);
  if (!result.requiredApprovalScopes.every((scope) => scopes.has(scope))) {
    throw new ApplicationError("APPROVAL_REQUIRED", "The supplied approval does not cover every required scope.", {
      details: { approvalId },
    });
  }
  await dependencies.approvals.consume(approvalId, WriteConditions.matchRevision(stored.revision), context, transaction);
}
...
export async function collectPortPages<Value>(
  load: (cursor: string | undefined) => Promise<Readonly<{ items: readonly Value[]; nextCursor?: string }>>,
  maximumPages = 10_000,
  ...
  throw new ApplicationError("RESOURCE_EXHAUSTED", "Pagination exceeded the maximum supported page count.", {
