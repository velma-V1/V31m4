import { PracticeTask, type PracticeTask as PracticeTaskType, type PracticeTaskId } from "@v31m4/domain";
...
export async function stopIdlePractice(
  ...
  const updated = await dependencies.unitOfWork.execute(context, async (transaction) => {
    const stored = requireValue(await dependencies.practice.get(practiceTaskId, context, transaction), "Practice task does not exist.");
    const stopped = PracticeTask.stop(stored.value, traceArtifactIds);
    const saved = await dependencies.practice.save(stopped, WriteConditions.matchRevision(stored.revision), context, transaction);
  ...
  try {
    ...
    throw new ApplicationError("DEPENDENCY_FAILURE", "Practice stopped but its isolated workspace could not be disposed.", {
      cause: error,
