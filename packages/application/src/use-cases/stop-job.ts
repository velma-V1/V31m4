import { Job, type Job as JobType, type JobId } from "@v31m4/domain";
...
export async function stopJob(
  ...
  const stored = await dependencies.unitOfWork.execute(context, async (transaction) => {
    ...
    const stopping = command.mode === "finish_and_stop"
      ? Job.beginFinishStop(current.value, { eventId: command.stopEventId, occurredAt: dependencies.clock.now() })
      : Job.beginEmergencyStop(current.value, { eventId: command.stopEventId, occurredAt: dependencies.clock.now() });
    const updated = await dependencies.jobs.save(stopping.job, WriteConditions.matchRevision(current.revision), context, transaction);
  ...
  try {
    ...
    await dependencies.unitOfWork.execute(context, async (transaction) => {
      const current = requireValue(await dependencies.jobs.getById(command.jobId, context, transaction), "Stopping job disappeared after kernel failure.");
    ...
    throw new ApplicationError("DEPENDENCY_FAILURE", "Production kernel could not stop the job.", { cause: error });
  }
}
