...
import type { ModelGatewayPort, ModelInvocationRequest, ModelInvocationResult } from "../ports/model-gateway.port.js";
...
export async function invokeModel(
  ...
  try {
    ...
    await dependencies.unitOfWork.execute(context, (transaction) => appendAudit(dependencies.audit, dependencies.clock, {
      ...
      resourceType: "model",
      resourceId: request.modelId,
      outcome: result.finishReason === "cancelled" ? "cancelled" : result.finishReason === "failed" ? "failed" : "completed",
