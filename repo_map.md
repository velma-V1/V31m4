...
├── contracts/                           # Layer 3: strict APIs, events, manifests, RPC 1.0 + additive negotiated RPC 1.1, + forbidden-key guard
├── application/
    ├── src/
...
apps/
├── runtime/                             # Layer 10: authoritative runtime plus explicit hermetic_reference/supervised_local composition; src/autonomy/ owns the 19-operation catalog, the canonical policy-backed capability authorizer, the governed agent-turn loop, and the Manager/Executor/Auditor role harness
...
| Supervised model discovery/routing | VERIFIED REAL — authenticated provider-neutral catalog with strict external cursors and bounded complete provider-page traversal, dynamic Ollama profiles, verified-measurement-first bounded routing, retryable escalation with exact candidate provenance, loopback OpenAI-compatible contract, and real installed `qwen3:8b` plus `qwen2.5-coder:14b` proof |
