...
| `/packages/infrastructure/src/sandbox` | Sandbox infrastructure | V31M4-owned `SandboxPort` lifecycle: an issued single-use capability, an authoritative `WorkspaceManagerPort` re-read under an execution-lease interlock that orders seal/discard against in-flight effects, hardened path containment, compare-and-apply as the only workspace write, the hermetic non-effecting reference backend, and the hardened direct-Docker challenger — settings validated before use, every docker invocation under the Layer 8 `ProcessSupervisor`, container lifecycle proven rather than inferred from an exit code, and lifecycle state retained when cleanup cannot prove its outcome; backend selection stays open until a target-host bake-off |
