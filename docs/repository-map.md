...
| `/apps/runtime/src/autonomy` | Autonomy runtime | The single V31M4-owned semantic operation catalog (19 model-facing operations, deliberately no `git.worktree`) plus the mandatory semantic authorization boundary: it snapshots the request, evaluates the existing `PolicyEnginePort`, refuses on an unsatisfied evidence precondition, binds immutable policy provenance and canonical risk/evidence/resource policy into the capability, derives trusted execution from the catalog, permits caller-supplied commands only for `command.run`, and attaches the workspace currency precondition the sandbox re-verifies at dispatch |
| `/apps/runtime/src/autonomy/role-manifest.ts` | Autonomy runtime | Role invocation manifests: `readOnly` derived from the role rather than believed, operation-level permissions checked against the closed catalog, a read-only role refused every write/execute/network-effect operation, and full context/model/skill/harness identity fingerprinted and expressible as ordinary Ledger resource facts |
| `/apps/runtime/src/autonomy/task-executor.ts` | Autonomy runtime | The fresh Executor: consumes the Manager's fingerprinted handoff whole (model, operations, skills, reasoning policy, and turn budget are read from it and never from the caller), verifies the dispatch and the frozen acceptance contract against it, re-reads authoritative state and fails closed if the capsule or workspace moved since selection, records the invocation as ordinary observed facts, and returns readiness for verification rather than completion |
| `/apps/runtime/src/autonomy/task-auditor.ts` | Autonomy runtime | The independent read-only Auditor: dispatched by its own freshly derived handoff rather than the Executor's, a deterministic verdict over the frozen contract, Evidence, and Ledger, a workspace identity that may be strengthened but never rebound or unbound, a separate fresh context that structurally cannot carry Executor output, and an optional model pass whose result is advisory and never consulted when the verdict is formed |
...
| `/packages/application/src/services/evidence-precondition.ts` | Application services | The deterministic evidence precondition: a requirement over existing `EvidenceRecord` kind/subject/status semantics or a valid Ledger observation, currency decided by the canonical `isEntryStillValid` rule, an unsatisfied verdict naming every missing requirement, and a typed non-retryable denial |
| `/apps/runtime/src/autonomy/evidence-precondition-catalog.ts` | Autonomy runtime | Which facts each semantic operation needs, resolved from operation, task class (the capsule's own phase), and risk. Every requirement names something the current governed path can honestly establish; reads and the operations that produce evidence are ungated; `command.run` inherits the union of every *executable* operation's requirements so the escape hatch can never undercut a semantic gate |
| `/apps/runtime/src/autonomy/governed-observation.ts` | Autonomy runtime | What a completed governed read established, derived by the runtime from the backend's own result and recorded as an `observation` — the half of evidence-conditioned effects that makes the gate reachable, since the investigation an agent performs produces the facts its later effect consumes. Never sourced from a model turn or a caller-supplied probe |
| `/apps/runtime/src/autonomy/evidence-precondition-gate.ts` | Autonomy runtime | The runtime half: reads the authoritative capsule, Ledger, and Evidence, evaluates the predicate, refuses before authority is minted, and writes nothing. Fail-closed on a missing capsule and on an unobserved resource |
| `/packages/application/src/services/role-handoff.ts` | Application services | The immutable, fingerprinted Manager→role dispatch: capsule, workspace, and acceptance-contract identity plus the canonicalised role/model/skill/operation/context policy, the Auditor handoff derived afresh from the frozen contract and authoritative result state, and the entry-state and result-state currency checks that make a stale or substituted dispatch fail closed |
| `/docs/reviews/autonomy-task5-role-harness-evidence.md` | Architecture governance | Task 5 role-harness evidence: role separation, deterministic-first routing, the frozen Entry Acceptance Snapshot, the resolved deferred role assignment, restart-safe handoff, and the round-2 independent-review repair (Manager-owned handoff, TOCTOU binding, audit workspace identity) |
