...
| `/packages/application/src/ports/sandbox.port.ts` | Application boundary | Typed sandbox lifecycle plus the fail-closed `SandboxIsolationPolicy` the public `ResourceBudget` cannot express; keeps the internal `unknown` effect status out of the public v1 tool status |
...
| `/packages/infrastructure/src/sandbox` | Sandbox infrastructure | V31M4-owned `SandboxPort` lifecycle bound to `WorkspaceManagerPort` authority, the hermetic non-effecting reference backend, and the hardened direct-Docker challenger backend; backend selection stays open until a target-host bake-off |
...
| `/apps/runtime/src/autonomy` | Autonomy runtime | The single V31M4-owned semantic operation catalog (19 model-facing operations, deliberately no `git.worktree`), operation-level role permissions, `code.patch` fingerprint/path-scope and staleness rules, and the fail-closed effect gate requiring policy + assigned workspace + sandbox |
