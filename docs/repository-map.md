...
| `/apps/runtime/src/autonomy/task-executor.ts` | Autonomy runtime | The fresh Executor: proves the frozen acceptance contract before the model is asked anything, offers only its minted manifest's operations, records the invocation as ordinary observed facts, and returns readiness for verification rather than completion |
| `/apps/runtime/src/autonomy/task-auditor.ts` | Autonomy runtime | The independent read-only Auditor: a deterministic verdict over the frozen contract, Evidence, and Ledger, a separate fresh context that structurally cannot carry Executor output, and an optional model pass whose result is advisory and never consulted when the verdict is formed |
...
| `/docs/reviews/autonomy-task5-role-harness-evidence.md` | Architecture governance | Task 5 role-harness evidence: role separation, deterministic-first routing, the frozen Entry Acceptance Snapshot, the resolved deferred role assignment, and restart-safe handoff |
