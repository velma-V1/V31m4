...
| `/apps/runtime/src/autonomy/evidence-precondition-catalog.ts` | Autonomy runtime | Which facts each semantic operation needs, resolved from operation, task class (the capsule's own phase), and risk. Reads and the operations that produce evidence are ungated; `command.run` inherits the union of every other operation's requirements so the escape hatch can never undercut a semantic gate |
