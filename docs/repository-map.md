...
| `/apps/runtime/src/job-command-surface.ts` + `job-start-command.ts` + `job-verification-plan.ts` | Runtime job command composition | Direct `job.start`/`job.execute` registration, explicit durable idempotency, runtime-owned execution claim/reconciliation, build-packet preparation, deterministic verification identity, repair composition, and champion/delivery finalization |
