...
| `/packages/application/src/ports/semantic-execution-capability.ts` | Application boundary | Closure-owned mint/verify/consume authority for `AuthorizedSemanticExecutionPlan`: no public factory, issuer-bound authenticity, single-use consumption, and the workspace currency precondition a sandbox re-verifies at dispatch |
...
| `/packages/infrastructure/src/processes` | Process infrastructure | Layer 8 supervised child-process lifecycle, process-group shutdown, explicit environment inheritance, cancellation, timeout, bounded output handling, and the termination reason callers need to reconcile external side effects |
...
| `/packages/infrastructure/src/sandbox` | Sandbox infrastructure | V31M4-owned `SandboxPort` lifecycle bound to `WorkspaceManagerPort` authority and to an issued authorization plan, the hermetic non-effecting reference backend, and the hardened direct-Docker challenger backend — settings validated before use, every docker invocation under the Layer 8 `ProcessSupervisor`, container removal verified, and lifecycle state retained when cleanup cannot prove its outcome; backend selection stays open until a target-host bake-off |
...
| `/apps/runtime/src/autonomy` | Autonomy runtime | The single V31M4-owned semantic operation catalog (19 model-facing operations, deliberately no `git.worktree`) plus `semantic-execution-authorization.ts`, the one mandatory boundary: it owns the capability mint in a closure and exposes only `authorize` and a verifier, derives the contract and trusted command from the catalog rather than the caller, permits a caller-supplied command only for `command.run`, and attaches the workspace currency precondition the sandbox re-verifies at dispatch |
