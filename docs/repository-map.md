...
| `/packages/application/src/ports/sandbox.port.ts` | Application boundary | Typed sandbox lifecycle, the fail-closed `SandboxIsolationPolicy` the public `ResourceBudget` cannot express, and the unforgeable `AuthorizedSemanticExecutionPlan` capability token the execution sink accepts in place of an operation name plus free-form JSON; keeps the internal `unknown` effect status out of the public v1 tool status |
...
| `/apps/runtime/src/autonomy` | Autonomy runtime | The single V31M4-owned semantic operation catalog (19 model-facing operations, deliberately no `git.worktree`) plus `semantic-execution-authorization.ts`, the one mandatory boundary that issues a non-forgeable `AuthorizedSemanticExecutionPlan`: operation-level roles, `allow` policy, active assigned workspace, bound sandbox, a trusted runtime-derived command for every operation except the `command.run` escape hatch, and `code.patch` fingerprint/path-scope/staleness validation |
