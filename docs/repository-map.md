...
| `/apps/runtime/src/composition-root.ts` | Runtime composition | The one place concrete adapters are assembled and every real command handler is wired (`project.create`, `mission.submit`, `job.start`, `job.execute`); owns the P0 direct-command idempotency repair (`SqliteIdempotencyStore` usage, the `job.execute` claim transaction) and the test-only `CompositionOverrides` seam |
| `/apps/runtime/src/use-case-infrastructure.ts` | Runtime adapters | Layer 4 port adapters backing the real use-case commands: project/mission/job SQLite repositories, event bus, `ReferenceProductionKernel`, approval/audit stores, `passthroughUnitOfWork` |
| `/apps/runtime/src/job-execution-infrastructure.ts` | Runtime adapters | Candidate/evidence SQLite repositories, real isolated workspace filesystem (`LocalWorkspaceManager`), and the `ReferenceModelGateway`/`ReferenceVerifier` reference adapters driving `job.execute` |
| `/apps/runtime/public/index.html` | Operator UI | Minimal real browser UI (session token, health, project/mission/job-start/job-execute panels, SSE-over-fetch live event log); no framework/build step; HTTP/SSE wire format verified, browser-driven proof not yet performed |
| `/apps/runtime/tests` | Runtime verification | Command, P0 idempotency, negative-verification-path, evidence-durable-readback, and restart-recovery regression against a real HTTP server + real SQLite |
