...
| `/apps/runtime/src/job-command-surface.ts` + `job-start-command.ts` | Runtime job command composition | Direct `job.start`/`job.execute` registration, explicit durable idempotency, runtime-owned execution claim/reconciliation, build-packet preparation, profile-selected orchestration, verification/champion/delivery finalization |
| `/apps/runtime/src/supervised` | Runtime supervised boundary | Optional local profile composition, authoritative prompt/model/report artifact promotion, contained project-copy/context preparation, bounded kernel work-product materialization, and independent verifier evidence translation |
...
| `/adapters/local-supervised` | Optional local execution adapters | SQLite-free bounded JSON-RPC child hosts for real loopback Ollama inference, Stage 4 plus general manifest kernel lifecycles, atomic contained file effects, and independent permission-bounded Node verification |
